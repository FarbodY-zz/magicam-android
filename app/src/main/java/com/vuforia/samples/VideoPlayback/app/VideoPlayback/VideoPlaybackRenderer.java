...
public class VideoPlaybackRenderer implements GLSurfaceView.Renderer
{
        ...
        boolean playImmediately)
    {
        ...
        System.out.println("Movie name changed to :" + movieName);
    ...
    public void onSurfaceCreated(GL10 gl, EGLConfig config)
    {
        ...
        for (int i = 0; i < VideoPlayback.NUM_TARGETS; i++)
        {
            ...
            if (mVideoPlayerHelper[i] != null)
            {
                ...
                if (mLoadRequested[i])
                {
                    //System.out.println("SALAM: " + mMovieName[i]);
    ...
    void renderFrame()
    {
        ...
        for (int tIdx = 0; tIdx < state.getNumTrackableResults(); tIdx++)
        {
            ...
            if(!mMovieName[currentTarget].equals(imageTarget.getName()))
            {
                mVideoPlayerHelper[currentTarget].unload();

                mMovieName[currentTarget] = imageTarget.getName();

                mVideoPlayerHelper[currentTarget].load(imageTarget.getMetaData(),
                        mCanRequestType[currentTarget], mShouldPlayImmediately[currentTarget],
                        mSeekPosition[currentTarget]);
            }
